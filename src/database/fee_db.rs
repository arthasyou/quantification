...
pub async fn create_test_table() -> Result<()> {
    let query = "
    DEFINE TABLE IF NOT EXISTS fee SCHEMALESS PERMISSIONS FULL;

    DEFINE FIELD IF NOT EXISTS user_id ON TABLE fee TYPE string;
    DEFINE FIELD IF NOT EXISTS agent_id ON TABLE fee TYPE string;
    DEFINE FIELD IF NOT EXISTS amount ON TABLE fee TYPE decimal;
    DEFINE FIELD IF NOT EXISTS created_at ON TABLE fee TYPE datetime VALUE time::now() READONLY;
